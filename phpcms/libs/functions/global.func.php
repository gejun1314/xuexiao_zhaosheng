...
function stu_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
    if(defined('URLRULE') && $urlrule == '') {
        $urlrule = URLRULE;
        $array = $GLOBALS['URL_ARRAY'];
    } elseif($urlrule == '') {
    ...
    if($num > $perpage) {
        ...
        $curr=isset($_GET['page'])?$_GET['page']:1;
            ...
            $multipage .= ' <a href="'.pageurl($urlrule, $curr_page-1, $array).'" class="a1">'.'上一页'.'</a><ul>';

        for($i = 1; $i <= $pages; $i++){
            if($i == $curr) {
                $multipage .= '<li> <a href="'.pageurl($urlrule, $i, $array).'" id="current2">'.$i.'</a></li>';
            }else{
        ...
        $multipage .= '</ul><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.'下一页'.'</a>';
    }
    return $multipage;
}
